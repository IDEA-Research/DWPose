...
class BaseCocoStyleDataset(BaseDataset):
    ...
    def _load_annotations(self) -> Tuple[List[dict], List[dict]]:
        ...
        try:
            ...
            self._metainfo['CLASSES'] = None

        instance_list = []
        image_list = []

        if 'UBody' in self.ann_file:
            test_ubody_path = np.load('data/UBody/splits/intra_scene_test_list.npy')
            test_ubody_path = test_ubody_path.tolist()
        ...
        for img_id in self.coco.getImgIds():
            ...
            if 'UBody' in self.ann_file:
                ...
                if self.test_mode == False:
                    if video_name in self.test_ubody_path: continue
                else:
                    if video_name not in self.test_ubody_path: continue
